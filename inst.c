...
//huaiyi de zhi ling you mulh
  ...
  //I_type
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest)=((word_t)src1<(word_t)imm));  //seqz
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest)=((int)src1<0? ~(~src1>>BITS(s->isa.inst.val,25,20)):src1>>BITS(s->isa.inst.val,25,20)));  //X
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest)=src1<<BITS((s->isa.inst.val),25,20));  //X
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest)=src1>>BITS((s->isa.inst.val),25,20));  //X
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest)=(Mr(src1+imm ,1))); 
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest)=SEXT(Mr(src1+imm ,2),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest)=(Mr(src1+imm ,4))&65535);
  //B_type
  ...
  //r_type
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , r, R(dest)=src1<<(src2&31)); 
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , r, R(dest)=src1>>(src2&31));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , r, long long int t=(int)src1*(int)src2;R(dest)=(BITS(t,63,32)));  //X
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , r, R(dest)=src1^src2);
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , r, R(dest)=((int)src1<0? ~(~src1>>(src2&31)):src1>>(src2&31)));
  //S_type
